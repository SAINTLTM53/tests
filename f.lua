...
local GeneralTab = Window:AddTab("Skidded")
local MainBOX = GeneralTab:AddLeftTabbox("Main") do
    ...
    Main:AddToggle("VisibleCheck", {Text = "Visible Check", Default = SilentAimSettings.VisibleCheck}):OnChanged(function()
        SilentAimSettings.VisibleCheck = Togggles.VisibleCheck.Value
    end)
    ...
    Main:AddDropdown("Method", {AllowNull = true, Text = "Silent Aim Method(use Find partonray)", Default = SilentAimSettings.SilentAimMethod, Values = {
...
local FieldOfViewBOX = GeneralTab:AddLeftTabbox("Field Of View") do
    ...
    Main:AddToggle("MousePosition", {Text = "Show Silent Aim Target"}):AddColorPicker("MouseVisualizeColor", {Default = Color3.fromRGB(54, 57, 241)}):OnChanged(function()
        ...
        SilentAimSettings.ShowSilentAimTarget = Togggles.MousePosition.Value 
    end)
    local PredictionTab = MiscellaneousBOX:AddTab("Prediction")
    PredictionTab:AddToggle("Prediction", {Text = "Mouse.Hit/Target Prediction"}):OnChanged(function()
        SilentAimSettings.MouseHitPrediction = Togggles.Prediction.Value
    end)
...
resume(create(function()
    RenderStepped:Connect(function()
        ...
        if Toggles.Visible.Value then 
            fov_circle.Visible = Togggles.Visible.Value
...
oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, Index)
    if self == Mouse and not checkcaller() and Toggles.aim_Enabled.Value and 
       Options.Method.Value == "Mouse.Hit/Target" and getClosestPlayer() then
        local HitPart = getClosestPlayer()
        if Index == "Target" or Index == "target" then 
            return HitPart
        elseif Index == "Hit" or Index == "hit" then 
            return ((Toggles.Prediction.Value and 
                     (HitPart.CFrame + (HitPart.Velocity * PredictionAmount)))
                    or (not Toggles.Prediction.Value and HitPart.CFrame))
